  ...
  return (
    <section id="team" className="relative py-20 sm:py-28">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-block px-3 py-1 rounded-full border border-purple-500/30 bg-purple-500/10 mb-4">
            <span className="text-purple-400 text-xs font-mono uppercase tracking-wider">The Crew</span>
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4">
            <span className="text-white">Our </span>
            <span className="text-purple-400 font-mono">Organizing Team</span>
          </h2>
          <p className="text-[#94a3b8] max-w-2xl mx-auto">
            The dedicated team behind Cyber Security Week 2025 working to bring you an unforgettable experience
          </p>
        </motion.div>
        ...
        {faculty.length > 0 && (
          <div className="mb-12">
            <h3 className="text-lg font-semibold text-white font-mono mb-6 text-center">
              <GraduationCap className="w-5 h-5 inline mr-2 text-amber-400" />
              Faculty <span className="text-amber-400">Coordinators</span>
            </h3>
                ...
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      <h4 className="text-base font-semibold text-white truncate">{member.name}</h4>
                      <p className="text-xs text-amber-400 font-mono">{member.role}</p>
                      <p className="text-xs text-[#94a3b8] font-mono truncate">{member.department}</p>
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <div>
          <h3 className="text-lg font-semibold text-white font-mono mb-6 text-center">
            ...
            Student <span className="text-purple-400">Team</span>
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
            {students.map((member, idx) => {
              const roleColor = roleColors[member.role] || "bg-gray-500/10 text-gray-400 border-gray-500/20";
              return (
                <motion.div
                  key={member.id}
                  initial={{ opacity: 0, y: 20 }}
                ...
                >
                  ...
                  <span className={`inline-block px-2 py-0.5 rounded text-[10px] font-mono border ${roleColor} mb-1`}>
                    {member.role}
                  </span>
                  <p className="text-[10px] text-[#94a3b8] font-mono">{member.department}</p>
                  {member.linkedin && (
                    <a
                      href={member.linkedin}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1 mt-2 text-[#00d4ff] hover:text-[#00d4ff]/80 text-xs font-mono"
                    >
